...
Environment variables (loaded from `.env` in this directory):
    FASTVOICE_API_ENDPOINT  - FastVoice API base URL (e.g. http://localhost:8000)
    FASTVOICE_API_TOKEN     - API token sent as X-API-Key
...
from fastvoice_sdk._generated_models import UpdateWorkflowRequest

load_dotenv(Path(__file__).parent / ".env")

# Replace with the numeric ID of an existing agent in your FastVoice account.
WORKFLOW_ID = 0
...
def main() -> int:
    api_endpoint = os.environ.get("FASTVOICE_API_ENDPOINT", "http://localhost:8000")
    api_token = os.environ.get("FASTVOICE_API_TOKEN")

    if not api_token:
        print("FASTVOICE_API_TOKEN is required", file=sys.stderr)
        return 1
    ...
    with FastVoiceClient(base_url=api_endpoint, api_key=api_token) as client:
