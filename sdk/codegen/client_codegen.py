"""Generate the FastVoice Python SDK client mixin from filtered OpenAPI.
...
Request and response types come from the sibling model file produced by
``datamodel-codegen``. This script imports those names but does not generate
the models itself.
...
def _map_scalar(schema: dict[str, Any]) -> str:
    t = schema.get("type")
    if t in _TYPE_MAP:
        return _TYPE_MAP[t]
    ...
    for path, methods in spec.get("paths", {}).items():
        for verb, op in methods.items():
            ...
            for p in op.get("parameters") or []:
                py_t = _map_scalar(p.get("schema") or {})
                param = Param(
                    name=p["name"],
                    py_type=py_t,
...
`FastVoiceClient` mixes in this class to get HTTP methods for every route
...
from fastvoice_sdk._generated_models import (
{imports}
)


class _GeneratedClient:
    # `FastVoiceClient.__init__` installs `self._request` (see client.py).
