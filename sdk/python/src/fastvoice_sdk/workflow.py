...
Wire format matches `WorkflowGraphDTO` from `api/services/workflow/dto.py`
1:1, so `Workflow.to_json()` output can be round-tripped through
`WorkflowGraphDTO.model_validate` without further translation.
"""
...
class Workflow:
    """Typed builder that produces `WorkflowGraphDTO`-compatible JSON.
    ...
    def __init__(self, *, client: FastVoiceClient, name: str = "", description: str = ""):
        self._client = client
        self.name = name
    ...
    def add_typed(
        ...
        *,
        ...
        `fastvoice_sdk.typed` instead of string+kwargs.
    ...
    def to_json(self) -> dict[str, Any]:
        """Serialize to the `WorkflowGraphDTO` wire format.

        Passes directly through `WorkflowGraphDTO.model_validate` and the
