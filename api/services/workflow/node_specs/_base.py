...
def evaluate_display_options(
    ...
    Used directly by the FastHTML property renderer. The golden fixtures in
    `display_options_fixtures.json` lock the visibility semantics in place.
    """
    if rules is None:
        return True
...
class NodeSpec(BaseModel):
    ...
    icon: str  # semantic icon key used by renderers (e.g., "Play")
    version: str = "1.0.0"
    ...
    def to_mcp_dict(self) -> dict[str, Any]:
        ...
        still served verbatim to the FastHTML property renderer (REST
        `node-types` route) and Python SDK code generation, which need the
        dropped fields.
        """
        out: dict[str, Any] = {
            "name": self.name,
