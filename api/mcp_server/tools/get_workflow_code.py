"""MCP tool that returns a workflow as safe Python SDK code.
...
`workflow.workflow_definition`. That matches the editor's "whichever is the
...
from api.mcp_server.python_bridge import PythonBridgeError


@traced_tool
async def get_workflow_code(workflow_id: int) -> dict[str, Any]:
    """Return the workflow as Python SDK code the LLM can edit.

    Output shape:
        {"code": "<Python source>", "workflow_id": int, "version": "draft" | "published" | "legacy"}
