"""Tests for the Python runtime SDK (`fastvoice_sdk`).
...
Covers:
- Workflow builder round-trips through WorkflowGraphDTO validation
...
from fastvoice_sdk._generated_models import NodeSpec
from fastvoice_sdk.errors import ValidationError
...
class _StubClient:
    """Stand-in for FastVoiceClient backed by the in-process spec registry.
...
def test_webhook_complex_fields_validate(client: _StubClient):
    ...
    WorkflowGraphDTO.model_validate(payload)
    _ = wh
