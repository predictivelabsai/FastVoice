...
from api.services.workflow.dto import WorkflowGraphDTO, sanitize_workflow_definition
...
async def test_dto():
    ...
    with open(_FIXTURES_DIR / "sample_branching_workflow.json", "r") as f:
        dto = WorkflowGraphDTO.model_validate_json(f.read())
    assert dto is not None


def test_dto_ignores_legacy_unknown_node_data_fields():
    dto = WorkflowGraphDTO.model_validate(
        {
            "nodes": [
                {
                    "id": "n1",
...
def test_sanitize_strips_ui_runtime_fields():
        ...
        "nodes": [
            {
                ...
                "type": "startCall",
                "position": {"x": 0, "y": 0},
                "width": 200,  # canvas-computed, preserved
                "selected": True,  # canvas runtime, preserved
                "data": {
                    "name": "Start",
                    "prompt": "hi",
                    "greeting": "hello",
    ...
    # Canvas runtime fields on the node itself are preserved.
