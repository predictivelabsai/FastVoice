...
app = FastAPI(
    title="FastVoice API",
    description="API for the FastVoice open voice automation platform",
    version="1.0.0",
    ...
    lifespan=lifespan,
    servers=[
        {"url": "https://voice.fastsme.com", "description": "Production"},
...
async def handle_mps_unavailable_error(
    ...
    """Tell callers this is a platform outage, not invalid customer config."""
...
# Register dotted public assets on the parent application. FastHTML includes a
# convenient extension route for static files, so these canonical root paths
# must take precedence over the mounted application.
@app.get("/healthz", include_in_schema=False)
async def fastvoice_health() -> dict[str, str]:
    return {"status": "ok", "service": "fastvoice"}


@app.get("/robots.txt", include_in_schema=False, response_class=PlainTextResponse)
async def fastvoice_robots() -> str:
    return "User-agent: *\nAllow: /\nSitemap: https://voice.fastsme.com/sitemap.xml\n"


@app.get("/sitemap.xml", include_in_schema=False)
async def fastvoice_sitemap() -> Response:
    body = """<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://voice.fastsme.com/</loc></url><url><loc>https://voice.fastsme.com/developers</loc></url></urlset>"""
    return Response(body, media_type="application/xml")

# The FastHTML surface is mounted last so the API, MCP and WebSocket routes
# retain priority while the public site and authenticated dashboard share the
# same origin, process and session boundary.
