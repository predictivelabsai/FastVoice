...
async def _validate_workflow_definition(
    ...
    # ----------- DTO Validation ------------
    dto: Optional[WorkflowGraphDTO] = None
    try:
        dto = WorkflowGraphDTO.model_validate(workflow_definition)
...
async def create_workflow_from_template(
    ...
    """Create a useful starter graph locally from a short brief.

    The upstream hosted template generator is intentionally not used: FastVoice
    does not send workflow descriptions or credentials to a managed service.
    """
    try:
        use_case = request.use_case.strip() or "Voice assistant"
        activity = request.activity_description.strip() or (
            "Understand the caller's request, help accurately, and confirm the outcome."
        )
        direction = "incoming caller" if request.call_type == "inbound" else "recipient"
        workflow_data = {
            "name": f"{use_case} · {request.call_type.title()}",
            "workflow_definition": {
                "nodes": [
                    {"id": "start", "type": "startCall", "position": {"x": 80, "y": 180}, "data": {"name": "Welcome", "prompt": f"Greet the {direction} and explain that you are calling about {use_case}."}},
                    {"id": "agent", "type": "agentNode", "position": {"x": 400, "y": 180}, "data": {"name": use_case, "prompt": activity}},
                    {"id": "end", "type": "endCall", "position": {"x": 720, "y": 180}, "data": {"name": "Close", "prompt": "Summarize the outcome, confirm any next step, and say goodbye."}},
                ],
                "edges": [
                    {"id": "start-agent", "source": "start", "target": "agent", "data": {"label": "continue", "condition": "always"}},
                    {"id": "agent-end", "source": "agent", "target": "end", "data": {"label": "complete", "condition": "always"}},
                ],
            },
        }
...
async def update_workflow(
    ...
    try:
        ...
        # nodes wholesale from the canvas store, which carries those.
