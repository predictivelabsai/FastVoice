...
async def transcribe_audio(
    ...
    """Reject the retired managed transcription shortcut.

    Live workflows still transcribe through their configured BYOK STT service.
    Uploaded-file transcription will return when it can use that same provider
    contract without routing customer audio through a hosted proxy.
    """
    del file, language, user
    raise HTTPException(
        status_code=501,
        detail="Uploaded-file transcription is not available in BYOK mode",
    )
